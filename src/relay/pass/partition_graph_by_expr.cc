...
public:
	...
	bool VisitExpr(const Expr &expr, const Expr &sexpr) final {
		...
		if (it != memo_.end()) {
			...
			bool is_in_subgraph;
			if (!expr.as<ConstantNode>() && sexpr.as<VarNode>()) {
	...
	bool VisitExpr_(const CallNode *call, const Expr &sexpr) final {
		...
		if (!VisitExpr(call->op, scall->op)) // here we compare call nodes' ops instead of its attributes.
			return false;
		if(!AttrsComparor::CompareNonDefault(call->attrs, scall->attrs))
			return false;
...
class GraphPartitionerByExpr: public SubgraphPartitionor{
public:
...
Expr PartitionGraphByExpr(const Expr &subexpr, const PrimExpr func_name,
		const int &device_type, const DataType &data_type, const Expr &expr,
		const IRModule model) {

	if(subexpr.as<FunctionNode>())
		return GraphPartitionerByExpr(model, subexpr.as<FunctionNode>()->body, func_name, device_type,
					data_type).Partition(expr);
	else
		return GraphPartitionerByExpr(model, subexpr, func_name, device_type,
					data_type).Partition(expr);
}

namespace transform {
Pass PartitionGraphByExpr(Expr subexpr, PrimExpr func_name, int device_type,
		DataType data_type) {
	runtime::TypedPackedFunc<Function(Function, IRModule, PassContext)> pass_func =
			[=](Function f, IRModule m, PassContext pc) {

				return Downcast<Function>(
						PartitionGraphByExpr(subexpr, func_name, device_type,
								data_type, f, m));
			};
